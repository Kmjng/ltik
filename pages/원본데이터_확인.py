...
def main():
    ...
    if st.sidebar.button("📊 데이터 조회", type="primary", use_container_width=True):
        with st.spinner("데이터를 조회하는 중..."):
            df = fetch_literature_data(
                ...
                country_filter, genre_filter, year_filter
            )
    ...
    with st.expander("ℹ️ 사용법 안내"):
        ...
        ### ✨ 주요 특징
        - **통합 필터링**: 모든 필터 조건을 한 번에 설정하여 원하는 데이터만 조회
        - **효율적 검색**: 데이터베이스 단에서 필터링하여 빠른 조회 성능
        - **조건별 파일명**: 다운로드 파일명에 적용된 필터 조건이 자동 포함
        - **실시간 옵션**: 데이터베이스의 최신 국가/장르 목록을 실시간 반영
        
        ### ⚠️ 주의사항
        - 발간일 기준으로 기간 필터링이 적용됩니다
        - 여러 필터를 동시 적용하면 교집합 결과가 조회됩니다
        - 장르는 genre1~4 컬럼 중 하나라도 일치하면 포함됩니다
        """)
